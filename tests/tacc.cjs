...
console.log('\n── 税理士先生へお渡しする表（acCsv）──');
{
  ...
  eq(l[1], '"2026年8月","2026年9月15日","内わけ","カルムコート東棟",780000',
     '内わけの行');
...
}
{
  ...
  const naka = l.filter(x => x.indexOf('"内わけ"') >= 0).length;
  eq(naka, 3, '内わけの行は3本（8月1本・9月2本）');
}
{
  const l = box.acCsv([{ ym:'2026年8月', sokinDate:'', total:null,
    rows:[{ label:'かぎ"かっこ"', amount:-1 }, { label:'カンマ,入り', amount:2 }] }])
    .split('\r\n');
  eq(l[1], '"2026年8月","","内わけ","かぎ""かっこ""",-1',
     '★引用符は2つにして escape する／マイナスは負の数のまま');
  eq(l[2], '"2026年8月","","内わけ","カンマ,入り",2', '★カンマで列が割れない');
...
}
{
  ...
  ok(m.body.indexOf('表（CSV）を添付') > 0, '★そのときは「表（CSV）」だけ');
}
{
  const m = box.acMail({}, '2026年8月', 'IREライフ株式会社', true);
  ok(m.body.indexOf('表（CSV）と明細書（PDF）を添付') > 0,
     '★PDFを付けられるときは、両方を書く');
}
{
